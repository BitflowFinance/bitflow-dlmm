...
(define-public (add-liquidity-multi
  ...
  (let (
    ...
    (asserts! (>= add-liquidity-result min-dlp-total) ERR_MINIMUM_LP_AMOUNT)
...
(define-public (withdraw-liquidity-multi
  ...
  (let (
    ...
    (asserts! (> (len positions) u0) ERR_EMPTY_POSITIONS_LIST)
    (asserts! (>= (get x-amount withdraw-liquidity-result) min-x-amount-total) ERR_MINIMUM_X_AMOUNT)
    (asserts! (>= (get y-amount withdraw-liquidity-result) min-y-amount-total) ERR_MINIMUM_Y_AMOUNT)
...
(define-public (move-liquidity-multi
  ...
  (let (
    ...
    (asserts! (> (len positions) u0) ERR_EMPTY_POSITIONS_LIST)
    (asserts! (>= move-liquidity-result min-dlp-total) ERR_MINIMUM_LP_AMOUNT)
...
(define-private (fold-add-relative-liquidity-multi
  ...
  (let (
    ...
    (add-liquidity-result (try! (contract-call? .dlmm-core-v-1-1 add-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) bin-id (get x-amount position) (get y-amount position) (get min-dlp position))))
    (updated-result (+ (unwrap! result ERR_NO_RESULT_DATA) add-liquidity-result))
  )
    (ok updated-result)
  )
...
(define-private (fold-move-relative-liquidity-multi
  ...
  (let (
    ...
    (move-liquidity-result (try! (contract-call? .dlmm-core-v-1-1 move-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) (get from-bin-id position) to-bin-id (get amount position) (get min-dlp position))))
    (updated-result (+ (unwrap! result ERR_NO_RESULT_DATA) move-liquidity-result))
  )
    (ok updated-result)
  )
