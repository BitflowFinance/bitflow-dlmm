...
(define-private (fold-add-relative-liquidity-multi
  ...
  (let (
    ...
    (add-liquidity-result (try! (contract-call? .dlmm-core-v-1-1 add-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) bin-id (get x-amount position) (get y-amount position) u1)))
    (updated-result (+ (unwrap! result ERR_NO_RESULT_DATA) add-liquidity-result))
  )
    (ok updated-result)
  )
...
(define-private (fold-move-liquidity-multi
  ...
  (let (
    (move-liquidity-result (try! (contract-call? .dlmm-core-v-1-1 move-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) (get from-bin-id position) (get to-bin-id position) (get amount position) u1)))
...
(define-private (fold-move-relative-liquidity-multi
  ...
  (let (
    ...
    (move-liquidity-result (try! (contract-call? .dlmm-core-v-1-1 move-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) (get from-bin-id position) to-bin-id (get amount position) u1)))
    (updated-result (+ (unwrap! result ERR_NO_RESULT_DATA) move-liquidity-result))
  )
    (ok updated-result)
  )
