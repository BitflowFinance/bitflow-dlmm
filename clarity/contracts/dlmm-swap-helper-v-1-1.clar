...
(define-private (fold-swap-helper
  ...
  (let (
    (swap-result (if (get x-for-y swap)
                     (try! (contract-call? .dlmm-core-v-1-1 swap-x-for-y (get pool-trait swap) (get x-token-trait swap) (get y-token-trait swap) (get bin-id swap) (get amount swap)))
                     (try! (contract-call? .dlmm-core-v-1-1 swap-y-for-x (get pool-trait swap) (get x-token-trait swap) (get y-token-trait swap) (get bin-id swap) (get amount swap)))))
